...
<head>
    ...
    <style>
        ...
        #modalTonton .modal-body {
            padding: 0; display: flex; justify-content: center; align-items: center; background: #000;
        }
        #wrapperEmbed { width: 100%; height: 80vh; display: flex; justify-content: center; }
        #wrapperEmbed iframe { width: 100%; height: 100%; border: none; }
        ...
        .btn-close-custom:hover { background: rgba(255, 255, 255, 0.4); transform: rotate(90deg); }

        @media (max-width: 576px) { #wrapperEmbed { height: 85vh; } }
    </style>
</head>
<body>
        ...
        <div class="col-md-10 glass-panel">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                <div class="d-flex gap-2">
                    <a href="detail-event.php" class="btn btn-outline-secondary rounded-pill px-3 py-2 d-flex align-items-center fw-bold shadow-sm">
            ...
            <div class="mb-3 d-flex align-items-center gap-2 flex-wrap">
                <a href="konten-manager.php" class="btn btn-sm <?= !isset($_GET['sort']) ? 'btn-success' : 'btn-outline-success' ?> rounded-pill px-3">Terbaru</a>
                <a href="konten-manager.php?sort=tinggi" class="btn btn-sm <?= (isset($_GET['sort']) && $_GET['sort'] == 'tinggi') ? 'btn-danger' : 'btn-outline-danger' ?> rounded-pill px-3">Level Tinggi</a>
                <a href="konten-manager.php?sort=rendah" class="btn btn-sm <?= (isset($_GET['sort']) && $_GET['sort'] == 'rendah') ? 'btn-primary' : 'btn-outline-primary' ?> rounded-pill px-3">Level Rendah</a>
            </div>
            ...
            <div class="table-responsive">
                <table class="table table-custom">
                    ...
                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) :
                        ...
                        if ($jumlah_scene >= 5) {
                            $level_label = "Level : 3"; $level_color = "bg-danger";
                        } elseif ($jumlah_scene >= 2) {
                            $level_label = "Level : 2"; $level_color = "bg-warning text-dark";
                        } else {
                            $level_label = "Level : 1"; $level_color = "bg-primary";
                        }
                    ?>
                        <tr class="row-main" data-bs-toggle="collapse" data-bs-target="#<?= $collapseID; ?>" aria-expanded="false">
                            ...
                            <td class="text-center">
                                <form method="POST" style="display:inline;" onsubmit="event.stopPropagation(); return confirm('Yakin hapus ide ini? Data tidak bisa dikembalikan!');">
                                        ...
                                        <div id="content-container-<?= $row['id']; ?>">
                                            <div class="mt-2 mb-3 pt-3 d-flex justify-content-between align-items-center">
                                                <?php if (!empty($row['referensi'])): ?>
                                                    <button class="btn btn-sm btn-info text-white rounded-pill px-3" onclick="tontonRef('<?= $row['referensi']; ?>')">
