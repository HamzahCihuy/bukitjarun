...
<style>
    ...
    /* Scrollbar Text Vertikal (Tipis) - Untuk isi pesan panjang */
    ...
    /* HIDE SCROLLBAR BAWAAN (Tapi tetap bisa discroll) */
...
<section id="menfess" class="py-20 relative bg-[#f8fafc]">
    ...
    <div class="container mx-auto px-4 relative z-10 max-w-7xl">
        ...
        <div class="w-full relative group/slider">
            ...
            <div id="menfess-wall" class="flex flex-row flex-nowrap gap-6 overflow-x-auto py-10 px-4 md:px-8 no-scrollbar scroll-smooth">
                
                <?php if(empty($list_menfess)): ?>
                    <div class="bg-white p-10 rounded-2xl border-2 border-dashed border-slate-300 text-center w-full md:w-[320px] shrink-0 mx-auto">
                        <p class="text-slate-400 font-bold">Belum ada curhatan. Jadilah yang pertama!</p>
                    </div>
                <?php endif; ?>

                <?php foreach($list_menfess as $row): $rot = rand(-2, 2); ?>
                <div class="relative group hover:z-50 transition-all duration-300 w-[280px] md:w-[320px] shrink-0 transform hover:-translate-y-2" 
                    ...
                    <div class="<?= $row['warna'] ?> p-6 rounded-bl-[30px] rounded-br-md rounded-tr-md shadow-md border border-black/5 group-hover:shadow-xl transition duration-300 relative min-h-[240px] flex flex-col justify-between">
                        
                        <div class="flex-1 mb-4">
                            <p class="font-hand text-xl md:text-2xl text-slate-800 leading-snug break-words max-h-[160px] overflow-y-auto pesan-scroll pr-2">
